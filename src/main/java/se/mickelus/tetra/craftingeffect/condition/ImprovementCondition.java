    ...
    public boolean test(Identifier[] unlocks, ItemStack upgradedStack, String slot, boolean isReplacing, Player player,
        ...
        if (anySlot) {
            return Arrays.stream(item.getMajorModules(upgradedStack))
                    ...
                    .anyMatch(module -> hasImprovement(upgradedStack, module));
        }
        ...
        return Optional.ofNullable(item.getModuleFromSlot(upgradedStack, slot))
                ...
                .map(module -> hasImprovement(upgradedStack, module))
                .orElse(false);
    }

    private boolean hasImprovement(ItemStack itemStack, ItemModuleMajor module) {
        if (level != null) {
            return level.test(module.getImprovementLevel(itemStack, improvement));
        }
        return module.getImprovement(itemStack, improvement) != null;
    }
}
