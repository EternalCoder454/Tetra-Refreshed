    ...
    public void draw(GuiGraphicsExtractor graphics, int refX, int refY, int screenWidth, int screenHeight, int mouseX, int mouseY, float opacity) {
        if (alignment == GuiAlignment.right) {
            ...
            for (int i = segmentCount; i < segmentCount + diffCount; i++) {
                drawSegmentReverse(graphics, refX, refY, i, diffColor, 1);
            }
            for (int i = segmentCount + diffCount; i < maxSegments; i++) {
                drawSegmentReverse(graphics, refX, refY, i, 0xffffff, 0.14f * opacity);
            }
        } else {
            for (int i = 0; i < segmentCount; i++) {
                drawSegment(graphics, refX, refY, i, 0xffffffff, opacity);
            }
            for (int i = segmentCount; i < segmentCount + diffCount; i++) {
                drawSegment(graphics, refX, refY, i, diffColor, 1);
            }
            for (int i = segmentCount + diffCount; i < maxSegments; i++) {
                drawSegment(graphics, refX, refY, i, 0xffffff, 0.14f * opacity);
            }
        }
    }
