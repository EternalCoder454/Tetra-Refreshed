...
public class StatGetterDeserializers {
    ...
    public static IStatGetter attributeGetter(JsonElement json) {
        ...
        Attribute attribute = RegistryHelper.get(BuiltInRegistries.ATTRIBUTE, Identifier.parse(data.attribute));
        if (attribute == null) {
    ...
    public static IStatGetter enchantmentGetter(JsonElement json) {
        ...
        Identifier enchantment = Identifier.parse(data.enchantment);
        if (enchantment == null) {
    ...
    record IsItemData(Identifier[] items, boolean inverted) {
    }
}
