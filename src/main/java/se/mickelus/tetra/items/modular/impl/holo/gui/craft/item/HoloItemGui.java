...
public class HoloItemGui extends GuiClickable {
    ...
    private final HolosphereEntryData data;
    private ItemStack workingStack;
    GuiElement slotGroup;
    private boolean isSelected = false;

    public HoloItemGui(int x, int y, HolosphereEntryData data, Runnable onSelect, Consumer<String> onSlotSelect) {
        ...
        icon = new GuiTexture(0, 0, 38, 38, data.icon.textureX, data.icon.textureY, data.icon.textureLocation);
        icon.setAttachment(GuiAttachment.middleCenter);
        addChild(icon);

        GuiElement labelGroup = new GuiElement(0, 0, 0, 0);
        String[] labelStrings = I18n.get("tetra.holo.craft." + data.key).split(" ");
        ...
        setupSlots(data.getAsModularItem(), data.getDefaultStack(), onSlotSelect);
